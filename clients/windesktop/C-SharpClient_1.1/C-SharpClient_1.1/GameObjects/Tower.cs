...
namespace C_SharpClient_1._1
{
    class Tower : GameObject
    {
        ...
        private SoundEffect soundProjectile;
        private int bulletSpeed;
        public Rectangle scaleTheBullet;

        private int attackUpdateCounter = 0;

        public Tower(Texture2D txt2D, Rectangle rec, int fireRate, double damage, double range, Texture2D trackProjtxt2D, int price ,SoundEffect soundProjectile, int bulletSpeed)
        {
            this.bulletSpeed = bulletSpeed;
            this.soundProjectile = soundProjectile;
            this.price = price;
        ...
        public override bool Update(ref List<GameObject> listToPrint)
        {
            ...
            if (attackUpdateCounter >= FireRate())
            {
                ...
                {
                    attackUpdateCounter -= FireRate();
                    soundProjectile.Play(0.2f, 0f, 0f);
                    proj = new TrackingProjectile(base.Rec.X + base.Rec.Width / 2, base.Rec.Y + base.Rec.Height / 2, 15, myCurrentTarget, damage, trackProjtxt2D, bulletSpeed);
                    if (scaleTheBullet.Width != 0)
                        proj.ScaleRectangle = scaleTheBullet;
                }
                else
                {
