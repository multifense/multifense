...
namespace C_SharpClient_1._1
{
    abstract class GameObject
    {
        ...
        public Rectangle ScaleRectangle { get { return scaleRectangle; } set { scaleRectangle = value; } }
        /// <summary>
        ...
        {
            if (scaleRectangle.Width == 0)
                spriteBatch.Draw(Text2D, new Rectangle(Rec.X + x, Rec.Y + y, Rec.Width, Rec.Height), color);
            else
                spriteBatch.Draw(Text2D, new Rectangle(Rec.X + x, Rec.Y + y, Rec.Width, Rec.Height),  new Rectangle(scaleRectangle.X, scaleRectangle.Y, scaleRectangle.Width, scaleRectangle.Height), color);
            if (font1text != null)
                if (setOutPutVector.Length() == 0)
                    spriteBatch.DrawString(font1text, strouput, new Vector2(this.rec.X + x, this.rec.Y + y-12), Color.Red);
                else
