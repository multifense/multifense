...
public class Kernel {
    ...
    public void towerDefinition(int type, String sprite, int damage, int timeBetweenShots, int range, int cost, int projectileSprite, int projectileSpeed, String projectileSound) {
    }
    ...
    public void victorWasDecided(int player) {
    	...
    	if (player == 0) {
    		...
    		session.pauseGame(GameScreen.WIN_SCREEN);
        } else {
            // DEFEAT!!!!
        	session.addGameText("WE HAVE LOST!");
        	session.pauseGame(GameScreen.LOSE_SCREEN);
        }
    ...
    public void playerWasDamagedByPlayer(int player, int opponent, int damage) {
        ...
        if (opponent == 0) {
        	...
        	giver.adjustPlayerHP(giver.getPlayerHP() + damage);
        }
    ...
    public void didFindGame(int myPlayerId, int startsInSeconds) {
    	...
    	session.getGameScreen().setScreenState(GameScreen.GETREADY_SCREEN_MP); // state 4 = start countdown
    }
    ...
    public void pathDefinition(int x, int y, int dirx, int diry, int length) {
    	this.session.getMap().getPath().addCodedPath(new Point((double) x, (double) y), new Point((double) dirx, (double) diry), length);
    }
